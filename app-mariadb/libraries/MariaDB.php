...
namespace clearos\apps\mariadb;
...
class MariaDB extends Daemon
{
    ...
    const COMMAND_MYSQLADMIN = '/usr/bin/docker exec -it mariadb /usr/bin/mysqladmin';
    const COMMAND_MYSQL = '/usr/bin/docker exec -it mariadb /usr/bin/mysql';
    ...
    public function is_password_set($username, $hostname)
    {
        ...
        $retval = $shell->execute(
            self::COMMAND_DOCKER, "exec -i mariadb /usr/bin/mysqladmin -u'$username' -h'$hostname' --protocol=tcp status", FALSE, $options
        );
    ...
    public function set_password($username, $old_password, $password, $hostname)
    {
        ...
        try {
            ...
            $shell = new Shell();
            $shell->Execute(
                self::COMMAND_DOCKER, "exec -i mariadb /usr/bin/mysqladmin -u'$username' $passwd_param -h'$hostname' --protocol=tcp password '$password'", FALSE, $options
            );
        ...
        try { 
            $shell->Execute(
                self::COMMAND_DOCKER, "exec -i mariadb /usr/bin/mysqladmin -u$username $passwd_param -h$hostname --protocol=tcp flush-privileges", FALSE, $options
            );
        } catch (Exception $e) {
